...
fun getPublicKeyRSA(entry: KeyStore.Entry?): PubKey {
    ...
    try {
        val pub = (entry as PrivateKeyEntry).certificate.publicKey
...
fun getPrivateKeyRSA(entry: KeyStore.Entry?): ByteArray {
    var exp = ByteArray(0)
    try {
        val private = (entry as PrivateKeyEntry).privateKey
        exp = (private as RSAPrivateKey).privateExponent.toByteArray()
    }
    catch (_: Exception) {
    }
    return exp
